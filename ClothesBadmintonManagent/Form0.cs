    ...
    public partial class Form0 : Form
    {
        ...
        private void btn_accept_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtB_UserName.Text))
            {
                ...
                txtB_UserName.Focus();
                return; // Stop processing if the username is invalid
            }

            // Email validation
            if (!txtB_email.Text.Contains("@"))
            {
                MessageBox.Show("Invalid email format.");
                txtB_email.Focus();
                return; // Stop processing if the email format is invalid
            }

            // Phone number validation
            if (txtB_Phone.Text.Length != 10 || !txtB_Phone.Text.All(char.IsDigit))
            {
            ...
            // Password validation
            if (!IsValidPassword(txtB_Password.Text))
            {
                MessageBox.Show("Password must contain at least one uppercase letter, one special character, and one number.");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    MessageBox.Show("Enter selection gender.");
